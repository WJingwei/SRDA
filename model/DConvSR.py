...
class DConvSR(nn.Module):
    EXPANSION=1
    def __init__(self, scale, n_channels=1, n_resblocks=32, n_feats=256, res_scale=0.1, stride=1,rgb_range=1):
        super(DConvSR, self).__init__()
        ...
        self.EXPANSION=1
        self.Dconv = DeformConv2d(self.n_channels, self.n_feats, kernel_size=3, padding=self.padding,stride=self.stride,bias=None,modulation=False)
        net_body = [
            ResBlock(
        ...
        net_tail = [
            ...
            nn.Conv2d(in_channels=self.n_feats, out_channels=self.n_channels,kernel_size=self.kernel_size, padding=self.padding)

        ]
        self.net_head = nn.Sequential(self.Dconv, Sam())
        self.net_body = nn.Sequential(*net_body)
        self.net_tail = nn.Sequential(*net_tail)

    def forward(self, x):
        x,x_map= self.net_head(x)
        res = self.net_body(x)
        res = torch.add(x, res)
        x = self.net_tail(res)
        return x,x_map
